...
export const fetchCompetitorData = async (): Promise<CompetitorData[]> => {
  await delay(800);
  const allResponses = getResponses();
  ...
  BANKS.forEach(b => {
    const tomMentions = allResponses.filter(r => {
      const input = (r.c1_first_bank || '').trim().toLowerCase();
      return b.name.toLowerCase().includes(input) || b.aliases?.some(a => a.toLowerCase() === input);
    }).length;
    tomScores[b.id] = (tomMentions / total) * 100;

    const awareCount = allResponses.filter(r => (r.c3_aware_banks || []).includes(b.id)).length;
    awarenessScores[b.id] = (awareCount / total) * 100;

    npsScores[b.id] = calculateNPSForSegment(allResponses, b.id);

    const considerCount = allResponses.filter(r => (r.c9_would_consider || []).includes(b.id)).length;
    considerationScores[b.id] = (considerCount / total) * 100;
  });

  return BANKS.map((b) => {
    const awareCount = allResponses.filter(r => (r.c3_aware_banks || []).includes(b.id)).length;
    const committed = allResponses.filter(r => r.c9_only_consider === b.id).length;
    const favors = allResponses.filter(r => (r.c9_favourites || []).includes(b.id)).length;
    const potential = allResponses.filter(r => (r.c9_would_consider || []).includes(b.id)).length;
    const rejectors = allResponses.filter(r => (r.c9_never_consider || []).includes(b.id)).length;
